...
/// show eg. some toggle button for folding the contents or other controls
/// and wrap all `components` representing [DocumentNode]s of this node
/// as well as all `childNodeWidgets`.
///
/// The builder function is not supposed to recursively also wrap widgets
...
/// in between and arranged in a folding tree.
///
...
/// [componentBuilders].
///
/// [SingleColumnFoldingLayout]'s `State` object implements [DocumentLayout],
...
/// to a [SingleColumnFoldingLayout], obtain its `State` object, and then
/// cast that `State` object to a [DocumentLayout].
class SingleColumnFoldingLayout extends StatefulWidget {
  const SingleColumnFoldingLayout({
    Key? key,
...
class _SingleColumnFoldingLayoutState extends State<SingleColumnFoldingLayout>
  ...
  @override
  void didUpdateWidget(SingleColumnFoldingLayout oldWidget) {
  ...
  DocumentPosition? getDocumentPositionAtOffset(Offset documentOffset) {
    editorLayoutLog
        .info('Getting document position at exact offset: $documentOffset');
  ...
  DocumentPosition? getDocumentPositionNearestToOffset(
    ...
    editorLayoutLog
        .info('Getting document position near offset: $documentOffset');
  ...
  Rect? getEdgeForPosition(DocumentPosition position) {
    ...
    if (component == null) {
      editorLayoutLog
          .info('Could not find any component for node position: $position');
      return null;
    }
  ...
  DocumentSelection? getDocumentSelectionInRegion(
    ...
    for (int i = componentSearchStartIndex;
      ...
      if (componentKey.currentState is! DocumentComponent) {
        editorLayoutLog
            .info(' - found unknown component: ${componentKey.currentState}');
        continue;
      }
    ...
    if (topNodeId == bottomNodeId) {
      ...
      return DocumentSelection(
        base: DocumentPosition(
          nodeId: topNodeId,
          nodePosition: topNodeBasePosition as NodePosition,
        ),
        extent: DocumentPosition(
          nodeId: bottomNodeId,
          nodePosition: topNodeExtentPosition as NodePosition,
        ),
      );
    } else {
      ...
      return DocumentSelection(
        base: DocumentPosition(
          ...
          nodePosition: (isDraggingDown
              ? topNodeBasePosition
              : bottomNodeBasePosition) as NodePosition,
        ),
        extent: DocumentPosition(
          nodeId: isDraggingDown ? bottomNodeId : topNodeId,
          nodePosition: (isDraggingDown
              ? bottomNodeExtentPosition
              : topNodeExtentPosition) as NodePosition,
        ),
      );
    }
  }
  ...
  DocumentComponent? getComponentByNodeId(String nodeId) {
    ...
    if (key == null) {
      editorLayoutLog
          .info('WARNING: could not find component for node ID: $nodeId');
      return null;
    }
